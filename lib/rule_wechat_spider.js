...
module.exports = {
    ...
    savePosts: function (rawList) {
        ...
        list.forEach(function (item) {
            ...
            msgInfo.newsId = item.comm_msg_info.id;
    ...
    writePost: function (msgInfo) {
        let contentUrl = msgInfo.content_url.replace(/amp;/g, "").replace(/\\\//g, "/");
        const shasum = crypto.createHash('sha1');
        let contentUrlSign = shasum.update(contentUrl).digest('hex');
        let post = {
            newsId: msgInfo.newsId,
            fileId: msgInfo.fileId || msgInfo.fileid,
            author: msgInfo.author,
            title: msgInfo.title.replace("&nbsp;", " "),
            contentUrl: contentUrl,
            contentUrlSign: contentUrlSign,
            biz: this.getBizFromUrl(contentUrl),
            mid: this.getMidFromUrl(contentUrl),
            cover: msgInfo.cover.replace(/\\\//g, "/"),
            digest: msgInfo.digest.replace("&nbsp;", " "),
            idx: msgInfo.idx,
            sourceUrl: msgInfo.source_url.replace(/\\\//g, "/"),
            createTime: new Date(msgInfo.datetime * 1000)
        }
        db.insertOne(post);
    },
